namespace UIWidgets
{
	...
	public static class LayoutUtilities
	{
		...
		{
			var ignorer = RECompatibility.GetComponent<ILayoutIgnorer>(target);
			...
			if (parent != null)
			{
				...
				{
					return RECompatibility.GetLayoutChildControlWidth(hv_layout_group);
				}
		...
		public static bool IsHeightControlled(RectTransform target)
		{
			...
			if (parent != null)
			{
				...
				if ((hv_layout_group != null) && hv_layout_group.enabled)
				{
					return RECompatibility.GetLayoutChildControlHeight(hv_layout_group);
				}
